...
GENSHIN_ACTIVE = "genshin.txt"
GENSHIN_EXPIRED = "genshin_exp.txt"
...
def verify_codes(active_set: set, expired_set: set) -> None:
    ...
    url = (
        base_url
        + "lang="
        + os.getenv("GENSHIN_LANG", "")
        + "&game_biz="
        + os.getenv("GENSHIN_GAMEBIZ", "")
        + "&uid="
        + os.getenv("GENSHIN_UID", "")
        + "&region="
        + os.getenv("GENSHIN_REGION", "")
        + "&cdkey="
    )
...
def write_expired(path: Path, expired_set: set) -> None:
    with (path / GENSHIN_EXPIRED).open("w", encoding="utf-8") as f:
...
def write_codes(path: Path, active_set: set) -> None:
    ...
    with (path / GENSHIN_ACTIVE).open("w", encoding="utf-8") as f:
...
def main(path: Path) -> None:
    expired_set = set()
    active_set = set()
    with (path / GENSHIN_EXPIRED).open() as f:
